...
class SkillLoader {
    constructor(extensionPath) {
        ...
        this.customPath = config.get('skillsPath') || undefined;
    }
    async loadSkill(cellCode) {
        ...
        for (const root of searchRoots) {
            ...
            try {
                ...
                const config = vscode.workspace.getConfiguration('cse');
                if (cellCode !== 'A1' && config.get('alwaysInjectTC', true)) {
    ...
    async loadMasterSkill() {
        return this.loadSkill('D5');
    }
    async loadSkillBundle(cells) {
        const parts = [];
