...
function activate(context) {
    const skillLoader = new skillLoader_1.SkillLoader(context.extensionPath);
    const boardProvider = new boardView_1.BoardViewProvider(context.extensionUri, BOARD_CELLS, skillLoader);
    // Register webview provider for the board
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('cse.boardView', boardProvider));
    ...
    // ── Commands ──────────────────────────────────────────────────────────
    context.subscriptions.push(vscode.commands.registerCommand('cse.openBoard', () => {
        vscode.commands.executeCommand('workbench.view.extension.cognitiveSkillsEngine');
    }));
    context.subscriptions.push(vscode.commands.registerCommand('cse.injectMasterSkill', async () => {
        const content = await skillLoader.loadMasterSkill();
        await deliverContent(content, 'MASTER-SKILL (Full System)');
        addToPath('D5');
    }));
    context.subscriptions.push(vscode.commands.registerCommand('cse.injectTokenCompression', async () => {
        const content = await skillLoader.loadSkill('A1');
        await deliverContent(content, 'Token Compression [A1]');
        addToPath('A1');
    }));
    context.subscriptions.push(vscode.commands.registerCommand('cse.injectSkill', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('cse.injectByCode', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('cse.routeTask', async () => {
        ...
        const confirm = await vscode.window.showQuickPick([`Inject: ${cells.join('·')}`, 'Cancel'], { placeHolder: `Auto-routed path: ${cells.join('·')}` });
    ...
    context.subscriptions.push(vscode.commands.registerCommand('cse.showPath', () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('cse.resetPath', () => {
        sessionPath = ['D5'];
        updatePathStatusBar();
    }));
    context.subscriptions.push(vscode.commands.registerCommand('cse.copyPath', () => {
...
async function deliverContent(content, label) {
    const config = vscode.workspace.getConfiguration('cse');
    const mode = config.get('insertMode', 'clipboard');
    if (mode === 'clipboard') {
...
function autoRoute(task) {
    ...
    if (arr.length > 1 && hasSci && !hasHum) {
        arr.push('D3');
    }
    if (arr.length > 1 && hasHum && !hasSci) {
        arr.push('D4');
    }
    return arr;
}
...
class SkillTreeProvider {
    ...
    getChildren(element) {
        if (!element) {
            return ['Core', 'Science', 'Humanities', 'Arts & Culture'].map(group => new SkillItem(group, vscode.TreeItemCollapsibleState.Collapsed, group));
        }
        const rowMap = {
            'Core': 'core', 'Science': 'science', 'Humanities': 'humanities', 'Arts & Culture': 'culture'
        };
        const row = rowMap[element.label];
        return Object.entries(this.cells)
            .filter(([, c]) => c.row === row)
            .map(([code, c]) => new SkillItem(`[${code}] ${c.symbol} — ${c.name}`, vscode.TreeItemCollapsibleState.None, row, code));
    }
}
class SkillItem extends vscode.TreeItem {
    constructor(label, collapsibleState, row, cellCode) {
